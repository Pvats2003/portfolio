...
**How I got the old site's content:** this session's network policy blocks the live site (`portfolio-seven-gold-54.vercel.app`), so I couldn't fetch it. Instead I took its content from this repo's data files (`data/*.ts` at commit `9b950a6`), which are what the old site is built from. If the live site runs an older commit, a few lines may differ. Nothing from the old design, layout or components is reused.
...
| Live at `city-ops-cf81f.web.app` (**link only after Q8**) | R-P1 |
...
| Live MVP at `karaval.base44.app` (**untested, Q5**) | R-P4 |
...
| R-P4 | Karavali: Community-Driven Local Discovery App · Solo founder · Apr 2026 – Present · Live MVP · karaval.base44.app |
